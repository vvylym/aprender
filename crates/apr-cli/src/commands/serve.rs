...
fn format_tools_prompt(tools: &[Tool]) -> String {
    if tools.is_empty() {
        return String::new();
    }
...
fn parse_tool_calls(output: &str) -> Option<Vec<ToolCall>> {
...
async fn safetensors_chat_completions_handler(
            ...
            let msgs: Vec<ChatMessage> = messages
                ...
                .filter_map(|m| {
                    Some(ChatMessage {
                        ...
                        tool_calls: None,
                        tool_call_id: m.get("tool_call_id").and_then(|t| t.as_str()).map(String::from),
                        name: m.get("name").and_then(|n| n.as_str()).map(String::from),
                    })
                })
                .collect();
            ChatCompletionRequest {
                model: request.get("model").and_then(|m| m.as_str()).unwrap_or("default").to_string(),
                messages: msgs,
                tools: request.get("tools").and_then(|t| serde_json::from_value(t.clone()).ok()),
                tool_choice: request.get("tool_choice").and_then(|t| serde_json::from_value(t.clone()).ok()),
    ...
    if has_tools {
        let tools_prompt = format_tools_prompt(parsed_request.tools.as_deref().unwrap_or(&[]));
    ...
    for msg in &parsed_request.messages {
        ...
        if msg.role == "system" && has_tools {
            ...
            prompt.push_str(&format_tools_prompt(parsed_request.tools.as_deref().unwrap_or(&[])));
