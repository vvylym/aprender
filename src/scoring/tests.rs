...
use super::*;
    ...
    fn test_acceptable_threshold_all_model_types() {
        ...
        assert!((ScoredModelType::LogisticRegression.acceptable_threshold() - 0.8).abs() < f32::EPSILON);
    ...
    fn test_quality_score_display_with_critical_issues() {
        let mut metadata = ModelMetadata::default();
        metadata.custom.insert("password".to_string(), "secret123".to_string());
    ...
    fn test_input_bounds_documented() {
        let mut metadata = ModelMetadata::default();
        metadata.custom.insert("input_bounds".to_string(), "0.0-1.0".to_string());
    ...
    fn test_input_schema_documented() {
        let mut metadata = ModelMetadata::default();
        metadata.custom.insert("input_schema".to_string(), "json".to_string());
    ...
    fn test_feature_ranges_documented() {
        let mut metadata = ModelMetadata::default();
        metadata.custom.insert("feature_ranges".to_string(), "[[0,1],[0,100]]".to_string());
    ...
    fn test_secret_detection_variants() {
        let test_keys = ["password", "secret", "api_key", "token", "PASSWORD", "SECRET", "API_KEY", "TOKEN"];
        ...
        for key in test_keys {
            ...
            assert!(score.has_critical_issues(), "Expected critical issue for key: {}", key);
        }
    }
    ...
    fn test_cv_score_low_std() {
        ...
        assert!(score.dimensions.accuracy_performance.breakdown.iter()
    ...
    fn test_quality_score_with_all_training_provenance() {
        ...
        assert!(score.dimensions.documentation_provenance.breakdown.iter()
    ...
    fn test_model_with_regularization() {
        ...
        metadata.hyperparameters.insert("alpha".to_string(), "0.01".to_string());

        let config = ScoringConfig::default();
        let score = compute_quality_score(&metadata, &config);

        // Should get regularization points
        assert!(score.dimensions.generalization_robustness.breakdown.iter()
