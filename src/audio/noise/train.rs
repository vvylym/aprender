...
impl NoiseTrainer {
    ...
    pub fn train_step(&mut self, configs: &[NoiseConfig], targets: &[Vec<f32>]) -> f32 {
        ...
        for (p, g) in w1.iter_mut().zip(grad_w1.iter()) {
            *p -= lr * g;
        }
        for (p, g) in b1.iter_mut().zip(grad_b1.iter()) {
            *p -= lr * g;
        }
        for (p, g) in w2.iter_mut().zip(grad_w2.iter()) {
            *p -= lr * g;
        }
        for (p, g) in b2.iter_mut().zip(grad_b2.iter()) {
            *p -= lr * g;
        }
        for (p, g) in w3.iter_mut().zip(grad_w3.iter()) {
            *p -= lr * g;
        }
        for (p, g) in b3.iter_mut().zip(grad_b3.iter()) {
            *p -= lr * g;
        }
