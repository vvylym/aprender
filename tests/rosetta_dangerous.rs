...
fn test_dimension_roundtrip_stability() {
    ...
    for (i, (&orig, &rec)) in test_data.iter().zip(recovered.iter()).enumerate() {
        assert_eq!(
            orig.to_bits(), rec.to_bits(),
            "Bit mismatch at index {}: {} != {}", i, orig, rec
        );
    }
}
...
fn test_error_accumulation_hypothesis() {
    ...
    for &n in &hops {
        ...
        eprintln!("n={}: measured={:.2e}, √n_bound={:.2e}, linear_bound={:.2e}",
    ...
    let log_errors: Vec<(f64, f64)> = errors.iter()
    ...
    if log_errors.len() >= 2 {
        ...
        assert!(slope < 0.8,
            "ERROR GROWTH IS SYSTEMATIC (slope={:.2} > 0.8). Investigate bias source!", slope);
    }
}
...
fn test_quantization_compatibility_documented() {
    ...
    assert!(true, "Type safety documented - see comments for compile_fail examples");
}
...
fn test_gguf_to_apr_direct() {
    ...
    eprintln!("Source GGUF: {} tensors, format: {:?}",
        inspection.tensors.len(), inspection.format);

    // Convert
    let report = rosetta.convert(&gguf_path, &apr_path, None)
        .expect("GGUF → APR conversion failed");

    eprintln!("Conversion: {} → {} in {}ms",
...
fn test_apr_to_gguf_direct() {
    ...
    if let Ok(data) = std::fs::read(&apr_path) {
        if data.len() < 4 || &data[0..3] != b"APR" {
            eprintln!("SKIP: APR file has invalid magic (got first bytes: {:?})", &data[0..4.min(data.len())]);
            let _ = std::fs::remove_dir_all(&temp_dir);
            return;
        }
    }
    ...
    match rosetta.convert(&apr_path, &gguf_out, None) {
        Ok(report) => {
            eprintln!("APR → GGUF: {} tensors in {}ms",
                report.target_inspection.tensors.len(),
                report.duration_ms);
            assert!(report.tensor_counts_match(),
...
fn test_safetensors_to_apr_direct() {
    ...
    let inspection = rosetta.inspect(&st_path).expect("Failed to inspect SafeTensors");
...
fn test_roundtrip_gguf_apr_gguf() {
    ...
    match rosetta.verify_roundtrip(&gguf_path, FormatType::Apr) {
        Ok(verification) => {
            ...
            assert!(verification.passes_with_tolerance(0.01),
...
fn test_chain_3hop() {
    ...
    let final_inspection = rosetta.inspect(&final_path)
        .expect("Failed to inspect final output");

    let original_inspection = rosetta.inspect(&gguf_path)
...
fn test_error_accumulation_real() {
    ...
    for (hops, error) in &errors {
        ...
        if *error > sqrt_n_bound * 2.0 {
            eprintln!("⚠️ SYSTEMATIC BIAS DETECTED at {} hops: error={:.2e} > bound={:.2e}",
