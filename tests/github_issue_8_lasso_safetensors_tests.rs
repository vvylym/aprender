...
fn test_lasso_save_load_roundtrip() {
    ...
    model.save_safetensors(path).unwrap();
    let loaded_model = Lasso::load_safetensors(path).unwrap();
...
fn test_lasso_multiple_save_load_cycles() {
    ...
    let model1 = Lasso::load_safetensors(path1).unwrap();

    // Cycle 2: loaded → save → load
    model1.save_safetensors(path2).unwrap();
    let model2 = Lasso::load_safetensors(path2).unwrap();

    // Cycle 3: loaded again → save → load
    model2.save_safetensors(path3).unwrap();
    let model3 = Lasso::load_safetensors(path3).unwrap();
...
fn test_lasso_different_alphas_produce_different_models() {
    ...
    let loaded1 = Lasso::load_safetensors(path1).unwrap();
    let loaded2 = Lasso::load_safetensors(path2).unwrap();
