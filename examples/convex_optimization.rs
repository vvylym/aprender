...
fn lasso_with_fista() {
    ...
    println!("Problem size: {} samples, {} features", m, n);
    println!("True sparsity: 3 non-zero coefficients");
    println!("Regularization parameter λ = {}\n", lambda);
...
fn nonnegative_least_squares() {
    ...
    let mut x_true_data = vec![0.0; n];
    for i in 0..n {
        x_true_data[i] = ((i + 1) as f32 * 0.5).exp() * 0.1;
    }
    let x_true = Vector::from_slice(&x_true_data);
    ...
    println!("Reconstruction error ‖Ax - b‖: {:.6}", recon_error);
...
fn high_dimensional_lasso_cd() {
    ...
    println!("Problem size: {} samples, {} features (n >> m)", m, n);
...
fn box_constrained_quadratic() {
    ...
    println!("\nConstraint satisfaction:");
    println!("Min coefficient: {:.6} (should be ≥ 0.0)", min_val);
...
fn comparison_fista_vs_cd() {
    ...
    println!("‖x_FISTA - x_CD‖: {:.6}", solution_diff);
